...
from utils.streamline_indices import write_list_of_streamline_indices, get_list_of_streamline_indices_from_mrtrix
...
def build_parser():
    ...
    p.add_argument('path_to_tractogram', help='Path to tractogram file which the streamline indices refer to.')
...
if __name__ == "__main__":
    ...
    idx_implausible, idx_plausible = get_list_of_streamline_indices_from_mrtrix(args['selection_file'])

    for suffix, idx_list in tqdm(list(zip([SUFFIX_PLAUSIBLE, SUFFIX_IMPLAUSIBLE], [idx_plausible, idx_implausible])),
                                 desc='Writing index files'):
        write_list_of_streamline_indices(
            args['output_basename'] + suffix + SUFFIX_FILE, idx_list, args['path_to_tractogram']
        )
