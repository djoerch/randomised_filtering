...
def build_argparser():
    ...
    p.add_argument('num_streamlines', type=int, help='Number of randomly chosen streamlines.')
    p.add_argument('output_file', help='Path to output file (.json).')
    p.add_argument('--hist', required=False, help='Path to histogram plot of the created indices.')
...
#    idx_list = (tf.header['nb_streamlines'] * np.random.rand(args['num_streamlines'])).astype('int32').tolist()
    idx_list = np.random.choice(np.arange(tf.header['nb_streamlines']), size=args['num_streamlines'], replace=False).tolist()
