...
# -- desc
# Main script for convenience. Allows to run all SIFT experiments for one subject, one after another.
# An option is provided to run the experiment on sequential sets of streamlines from a tractogram
# or randomized (rSIFT) s.t. the average streamline is evaluated 5 times fo each subset size.
#
...
if [ "$1" = "--help" ] ; then
  ...
  echo "First argument: path to folder with subject data, must contain tractogram (named all.trk) in trk format, diffusion data (data.nii.gz, as downloaded from Human Connectome Project ) and FODs (WM_FODs.mif, as created following the descriptions on https://doi.org/10.5281/zenodo.1477956)"
...
# change appropriately if your tractogram contains != 10M streamlines or you want more/less votes per streamlines
if [ "$2" = "1" ] ; then
